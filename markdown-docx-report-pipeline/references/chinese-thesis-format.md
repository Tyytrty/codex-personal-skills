...
- Image paragraph: centered; 5 pt before; keep the image and its caption together when pagination allows.
...
2. Export a PDF into a task-local hidden or temporary QA directory, not beside the final DOCX.
3. Render all PDF pages to PNG and inspect every page at 100% zoom.
4. Check fonts, margins, headers, footers, page numbers, figures, captions, three-line tables, clipping, overlaps, and avoidable blank pages.
5. Iterate and regenerate QA artifacts after every layout-sensitive change.
6. Deliver only the requested DOCX. Do not link or separately output the QA PDF, page PNGs, or contact sheets unless the user explicitly requests them.
