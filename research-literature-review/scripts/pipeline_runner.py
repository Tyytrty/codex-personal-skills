...
目标：生成 Markdown + BibTeX 综述，按首次被引顺序自动编号，校验关键原文图来源；不生成 Word。
"""
...
class PipelineRunner:
    ...
    STAGES = {
        ...
        "5_write": "Markdown 写作与首次被引定序",
        "6_validate": "校验（字数/引用顺序/章节/原文图）",
        "7_export": "整理 Markdown 交付",
    }
    ...
    def _write_working_conditions_skeleton(self, path: Path) -> None:
        ...
        sections = [
            ...
            "综述正文（{主题}_review.md）必须完全聚焦领域知识，不应出现任何以下内容：",
...
"""
    ...
    def run_stage_5_write(self) -> bool:
        print("\n[阶段5] Markdown 写作与引用定序")
        wc = self._output_path("working_conditions")
        review_draft_md = self._output_path("review_draft_md")
        review_md = self._output_path("review_md")
        review_assets = self._output_path("review_assets")
        references_bib = Path(self.state.output_files.get("references_bib", self._output_path("references_bib")))
        self.state.output_files["working_conditions"] = str(wc)
        self.state.output_files["review_draft_md"] = str(review_draft_md)
        self.state.output_files["review_md"] = str(review_md)
        self.state.output_files["review_assets"] = str(review_assets)
        ...
        print(f"    - {review_draft_md.name}（Markdown 草稿，使用 [@bibkey] 引用）")
        print(f"    - {review_assets.name}/（关键开放许可原文图）")
        ...
        if review_draft_md.exists() and references_bib.exists() and wc.exists():
            ok = self._run_script(
                "finalize_markdown_citations.py",
                ["--input", str(review_draft_md), "--bib", str(references_bib), "--output", str(review_md)],
            )
            if ok and review_md.exists():
                self.state.completed_stages.append("5_write")
                self.save_state()
                return True
        elif review_md.exists() and references_bib.exists() and wc.exists():
            self.state.completed_stages.append("5_write")
            self.save_state()
            return True

        print("  ✗ 尚未找到 Markdown 草稿或引用定序失败，请补全后重跑 --resume-from 5")
        self.save_state()
        return False
    ...
    def run_stage_6_validate(self) -> bool:
        ...
        review_md = Path(self.state.output_files.get("review_md", ""))
        references_bib = Path(self.state.output_files.get("references_bib", ""))
        if not wc.exists() or not review_md.exists() or not references_bib.exists():
            print("  ✗ 缺少工作条件/Markdown/BibTeX")
            return False
        ...
        validation_ok, validation_output = self._run_script_capture_output(
            "validate_review_md.py",
            [
                "--md",
                str(review_md),
                "--bib",
    ...
    def run_stage_7_export(self) -> bool:
        ...
        review_md = Path(self.state.output_files.get("review_md", ""))
        references_bib = Path(self.state.output_files.get("references_bib", ""))
        review_assets = Path(self.state.output_files.get("review_assets", ""))
        if not review_md.exists() or not references_bib.exists():
            print("  ✗ 缺少 review.md 或参考文献 BibTeX")
            return False
        forbidden_word = review_md.with_suffix(".docx")
        if forbidden_word.exists():
            print(f"  ✗ 检测到禁止的 Word 交付物: {forbidden_word}")
            return False
        review_assets.mkdir(parents=True, exist_ok=True)
        self.state.completed_stages.append("7_export")
        self.save_state()
        print(f"  ✓ Markdown: {review_md}")
        print(f"  ✓ BibTeX: {references_bib}")
        print(f"  ✓ 原文图目录: {review_assets}")
        return True
