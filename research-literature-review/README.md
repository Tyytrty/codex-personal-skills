...
> **当前默认输出（2026-07-18 起）**：以 `{主题}_review.md` 为唯一正式正文，不生成 Word。文内参考文献按首次被引顺序显示为数字编号，参考文献表按 GB/T 7714—2015 顺序编码制著录并同步排序；同时筛选并嵌入许可已核验的关键原文图。README 后文如仍出现 LaTeX/PDF/Word，均属于历史版本说明，以 `SKILL.md` 和 `config.yaml` 为准。
...
- 强制交付 Markdown；硬校验：必需章节、字数 min/max、参考文献数 min/max、首次被引编号顺序、GB/T 7714—2015 参考文献格式一致性和原文图来源许可。不生成 Word；PDF 仅按用户明确要求附加导出。
...
- **多语言支持**：支持将 Markdown 综述翻译为多种语言（en/zh/ja/de/fr/es），保留数字引用、图片路径和结构不变。
